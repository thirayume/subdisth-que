        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-col items-center space-y-2 p-4 bg-green-50 rounded-lg">
            <Avatar className="w-16 h-16">
              ...
              <AvatarFallback>{state.displayName?.charAt(0) || 'L'}</AvatarFallback>
            </Avatar>
            <h3 className="font-medium">{state.displayName}</h3>
            {state.statusMessage && (
              <p className="text-sm text-gray-600 text-center">{state.statusMessage}</p>
            )}
          </div>
          ...
          <form onSubmit={handleConnect} className="space-y-4">
            <div className="space-y-2">
              <label htmlFor="phone" className="text-sm font-medium text-gray-700">
                กรอกเบอร์โทรศัพท์ที่ลงทะเบียนในระบบ
              </label>
              <Input
                id="phone"
                type="tel"
