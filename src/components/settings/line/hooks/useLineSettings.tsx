...
export const useLineSettings = () => {
  ...
  // State declarations - Always at the top
  ...
  }, [validateSettings, lineSettings, ttsConfig]);
  ...
  // Register effects after all callback definitions
  useEffect(() => {
    const savedSettings = localStorage.getItem('lineSettings');
    if (savedSettings) {
      try {
  ...
  // Validate settings whenever they change during editing - after other effects
  useEffect(() => {
    if (isEditing) {
      validateSettings();
    }
