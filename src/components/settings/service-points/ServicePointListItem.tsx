...
import ServicePointDeleteDialog from './ServicePointDeleteDialog';

interface ServicePointListItemProps {
  servicePoint: ServicePoint;
  onEdit: (servicePoint: ServicePoint) => void;
  onDelete: (id: string) => Promise<void>;
}

const ServicePointListItem: React.FC<ServicePointListItemProps> = ({
  servicePoint,
  onEdit,
  onDelete,
}) => {
  return (
    <div className="flex items-center justify-between border-b py-4 last:border-0">
      ...
      <div className="flex space-x-2">
        ...
        <ServicePointDeleteDialog servicePoint={servicePoint} onDelete={onDelete} />
      </div>
    </div>
  );
};

export default ServicePointListItem;
